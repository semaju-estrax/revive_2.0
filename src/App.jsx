...
import { Recycle, Users, Trophy, Activity, Search, QrCode, ArrowRight, Trash2, XCircle, Lock, User, Plus, X, Edit, Trash, Gift, Wallet, BarChart3, LayoutDashboard, Calendar, Filter, Upload, Settings, Bell, CheckCircle, Clock, Home } from 'lucide-react';
...
export default function App() {
  ...
  const [showQrModal, setShowQrModal] = useState(false); // Modal untuk Papar QR Code Penduduk
  ...
  const [conversionRate, setConversionRate] = useState(0.10); // Default RM0.10 sebotol untuk komuniti
  ...
  const handleLogin = (e) => {
    ...
    if (loginTab === 'admin') {
      ...
      } else alert("ID Penduduk tidak dijumpai atau Kata Laluan salah (Gunakan default: 1234)");
    }
  };
  ...
  const requestRedeem = async () => {
    if (window.confirm("Hantar permohonan penebusan insentif tunai hijau anda kepada pihak pengurusan?")) {
      try {
      ...
      } catch (err) { alert("Ralat sistem menghantar permohonan."); }
    }
  };

  const handleAcceptRedeem = async (resident) => {
    const totalRM = (resident.marks * conversionRate).toFixed(2);
    if (window.confirm(`Sahkan pembayaran RM ${totalRM} kepada ${resident.name} (${resident.houseNo})?\n\nBaki ganjaran semasa akan dikosongkan.`)) {
      try {
        await updateDoc(doc(db, 'residents', resident.id), { marks: 0, redeemRequest: false });
        alert(`Pembayaran RM ${totalRM} berjaya disahkan!`);
      } catch (err) { alert("Ralat pengesahan ganjaran."); }
    }
  };
  ...
  const handleCancelRedeem = async (residentId) => {
    ...
    } catch (err) { alert("Gagal membatalkan permohonan."); }
  };

  const handleAddResident = async (e) => {
    e.preventDefault();
    if (!newResident.id || !newResident.name || !newResident.ramin || !newResident.houseNo) return alert("Sila lengkapkan profil penduduk!");
    try {
      ...
      setShowAddModal(false); setNewResident({ id: '', name: '', ramin: '', houseNo: '' }); alert("Penduduk berjaya didaftarkan!");
    } catch (err) { alert("Gagal menambah penduduk."); }
  };
  ...
  const handleUpdateResident = async (e) => {
    ...
    try {
      ...
      setShowEditModal(false); alert("Profil penduduk dikemas kini!");
  ...
  const handleUpdateSettings = async (e) => {
    ...
    try {
      ...
      alert("Kadar ganjaran kitar semula dikemas kini!");
    } catch (err) { alert("Gagal mengemas kini tetapan."); }
  };

  const handleDeleteResident = async (id, name) => {
    if (window.confirm(`Padam terus rekod penduduk ${name}?`)) {
      try { await deleteDoc(doc(db, 'residents', id)); } catch (err) { alert("Gagal memadam."); }
    }
  };

  // Simulasi jika kod QR penduduk diimbas oleh hardware IoT (ESP32 Smart Bin)
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans relative">
        ...
        <div className="fixed inset-0 bg-slate-900 bg-opacity-60 flex justify-center items-center z-50 p-4 backdrop-blur-sm">
          <div className="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div className="flex mb-6 bg-slate-100 p-1 rounded-lg">
              <button onClick={() => setLoginTab('resident')} className={`flex-1 py-2 text-sm font-bold rounded-md ${loginTab === 'resident' ? 'bg-white shadow text-emerald-600' : 'text-slate-500'}`}>Penduduk</button>
              <button onClick={() => setLoginTab('admin')} className={`flex-1 py-2 text-sm font-bold rounded-md ${loginTab === 'admin' ? 'bg-white shadow text-emerald-600' : 'text-slate-500'}`}>Pengurus Taman</button>
            </div>

            <form onSubmit={handleLogin} className="space-y-6">
              <div>
                <label className="block text-sm font-medium mb-1">{loginTab === 'resident' ? 'ID Penduduk (Cth: TM001)' : 'Username Admin'}</label>
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Kata Laluan {loginTab === 'resident' && <span className="text-slate-400 font-normal">(Default: 1234)</span>}</label>
                <div className="relative">
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-slate-900 bg-opacity-50 flex justify-center items-center z-50 p-4">
          <div className="bg-white p-6 rounded-xl w-full max-w-md">
            <div className="flex justify-between mb-4"><h3 className="font-bold text-xl">Tambah Akaun Penduduk</h3><button onClick={() => setShowAddModal(false)}><X/></button></div>
            <form onSubmit={handleAddResident} className="space-y-4">
              <input type="text" placeholder="ID Penduduk (Cth: TM105)" required value={newResident.id} onChange={(e) => setNewResident({...newResident, id: e.target.value})} className="w-full p-2 border rounded" />
              <input type="text" placeholder="Nama Penuh Ketua Rumah / Penduduk" required value={newResident.name} onChange={(e) => setNewResident({...newResident, name: e.target.value})} className="w-full p-2 border rounded" />
              <div className="grid grid-cols-2 gap-4">
                <select required value={newResident.ramin} onChange={(e) => setNewResident({...newResident, ramin: e.target.value})} className="w-full p-2 border rounded bg-white">
                  <option value="" disabled>Pilih Ramin</option>
                  {availableRamins.map(r => <option key={r} value={r}>Ramin {r}</option>)}
                </select>
                <input type="text" placeholder="No. Rumah (Cth: No. 42)" required value={newResident.houseNo} onChange={(e) => setNewResident({...newResident, houseNo: e.target.value})} className="w-full p-2 border rounded" />
              </div>
              <button type="submit" className="w-full bg-blue-600 text-white p-2 rounded font-bold">Simpan Akaun</button>
            </form>
          </div>
        </div>
      )}
      ...
      {showEditModal && (
        <div className="fixed inset-0 bg-slate-900 bg-opacity-50 flex justify-center items-center z-50 p-4">
          <div className="bg-white p-6 rounded-xl w-full max-w-md">
            <div className="flex justify-between mb-4"><h3 className="font-bold text-xl">Edit Akaun Penduduk</h3><button onClick={() => setShowEditModal(false)}><X/></button></div>
            <form onSubmit={handleUpdateResident} className="space-y-4">
              ...
              <div><label className="text-sm font-bold text-slate-600">Baki Botol Semasa</label><input type="number" required value={editResidentData.marks} onChange={(e) => setEditResidentData({...editResidentData, marks: e.target.value})} className="w-full p-2 border rounded font-bold text-blue-600" /></div>
              <div><label className="text-sm font-bold text-slate-600">Jumlah Kitaran Seumur Hidup</label><input type="number" required value={editResidentData.totalMarks} onChange={(e) => setEditResidentData({...editResidentData, totalMarks: e.target.value})} className="w-full p-2 border rounded font-bold text-emerald-600" /></div>
        ...
        <div className="max-w-7xl mx-auto flex justify-between items-center p-4">
          ...
          <div className="flex space-x-1 bg-emerald-700/50 p-1 rounded-lg">
            ...
            <button onClick={() => setActiveTab('statistik')} className={`px-3 py-1.5 rounded-md text-sm font-medium transition-all flex items-center ${activeTab === 'statistik' ? 'bg-white text-emerald-700 shadow-sm' : 'text-emerald-50 hover:text-white'}`}>
              <BarChart3 className="w-4 h-4 sm:mr-2" /> <span className="hidden sm:inline">Zon Statistik</span>
            </button>
          </div>
            ...
            {userRole !== 'guest' ? (
              <div className="flex items-center space-x-3">
                <span className="hidden md:block text-sm font-medium bg-emerald-800 px-3 py-1 rounded-full">
                  Hi, {userRole === 'admin' ? 'Pengurus Sektor' : loggedInResident.name}
                </span>
      ...
      <main className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
        ...
        {userRole === 'resident' && loggedInResident && (
          <div className="bg-white border border-slate-200 p-6 mb-8 rounded-2xl shadow-sm grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
            <div>
              <h2 className="text-xl font-bold text-slate-800 flex items-center"><Home className="mr-2 text-emerald-600" /> Akaun Kediaman Anda</h2>
              <p className="text-slate-500 text-sm mt-1">Sektor: <span className="font-bold text-slate-700">Ramin {loggedInResident.ramin}</span> | No. Rumah: <span className="font-bold text-slate-700">{loggedInResident.houseNo}</span></p>
              
              <div className="mt-4 flex space-x-4">
                <div>
                  <p className="text-xs text-slate-400 font-medium uppercase">Baki Mata Ganjaran</p>
                  <p className="text-2xl font-bold text-blue-600">{loggedInResident.marks} Botol</p>
                </div>
                <div>
                  <p className="text-xs text-slate-400 font-medium uppercase">Nilai Tunai Hijau</p>
            ...
            <div className="space-y-3">
              {/* BUTANG UTAMA UNTUK DISPLAY QR CODE SEMASA NAK BUANG BOTOL */}
              <button 
              ...
              >
                ...
                <span>Papar Kod QR Penduduk</span>
              </button>
              ...
              {loggedInResident.marks >= 10 && !loggedInResident.redeemRequest ? (
                ...
                <div className="bg-amber-50 text-amber-700 text-xs p-3 rounded-lg border border-amber-200 flex items-center">
                  ...
                  <span>Permohonan Penebusan Insentif Sedang Disemak oleh Pengurus Komuniti.</span>
                </div>
              ) : (
                <p className="text-xs text-center text-slate-400">Kumpul minimum 10 botol untuk tebus insentif tunai.</p>
              )}
            </div>
          </div>
        )}
        ...
        {userRole === 'admin' && pendingRequestsCount > 0 && (
          <div className="bg-blue-100 border-l-4 border-blue-500 p-4 mb-6 rounded-r-lg shadow-sm">
            <h3 className="font-bold text-blue-800 flex items-center"><Bell className="h-5 w-5 mr-2" /> Notifikasi Pengurusan Komuniti</h3>
            <p className="text-blue-700 text-sm mt-1">Terdapat <b>{pendingRequestsCount}</b> tuntutan ganjaran tunai hijau daripada penduduk yang sedia untuk diproses.</p>
          </div>
        )}
          ...
          <div className="animate-in fade-in duration-300">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
              ...
              {userRole === 'admin' && (
                <div className="flex flex-wrap gap-2">
                  <button onClick={() => { setTempRate(conversionRate); setShowSettingsModal(true); }} className="flex items-center px-4 py-2 rounded-lg font-medium shadow-sm bg-slate-200 text-slate-700 hover:bg-slate-300"><Settings className="mr-2 h-4 w-4" /> Tetapan Nilai</button>
                  <button onClick={() => setShowAddModal(true)} className="flex items-center px-4 py-2 rounded-lg font-medium shadow-sm bg-blue-600 text-white hover:bg-blue-700"><Plus className="mr-2 h-4 w-4" /> Tambah Penduduk</button>
                  <button onClick={simulateESP32Scan} disabled={isScanning} className="flex items-center px-4 py-2 rounded-lg font-medium shadow-sm bg-emerald-600 text-white hover:bg-emerald-700">
                    <QrCode className="mr-2 h-4 w-4" /> Simulasi Imbas IoT Smart Bin
                  </button>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6 flex items-center">
                ...
                <div><p className="text-sm text-slate-500 font-medium">Jumlah Botol Dikumpul (Taman Mawar)</p><h3 className="text-3xl font-bold text-slate-800">{totalBottlesLifetime}</h3></div>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6 flex items-center">
                <div className="bg-amber-100 p-4 rounded-lg text-amber-600 mr-4"><Wallet className="h-8 w-8" /></div>
                <div><p className="text-sm text-slate-500 font-medium">Jumlah Edaran Dana Lestari (RM)</p><h3 className="text-3xl font-bold text-slate-800">{(totalBottlesLifetime * conversionRate).toFixed(2)}</h3></div>
              </div>
              <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6 flex items-center">
                <div className="bg-blue-100 p-4 rounded-lg text-blue-600 mr-4"><Trophy className="h-8 w-8" /></div>
                <div>
                  <p className="text-sm text-slate-500 font-medium">Hero Lestari Taman Mawar</p>
                  ...
                  <p className="text-xs text-blue-600 font-medium">{topResident && getResidentTotal(topResident) > 0 ? `Total: ${getResidentTotal(topResident)} Botol (${topResident.houseNo})` : ''}</p>
                </div>
              </div>
            </div>
              ...
              <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
                <div className="p-4 border-b border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  <h2 className="text-xl font-bold text-slate-800 shrink-0">
                    {userRole === 'resident' ? `Kedudukan Sektor Ramin ${loggedInResident.ramin}` : 'Pangkalan Data Penduduk'}
                  </h2>
                  <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
                    {userRole !== 'resident' && (
                      <div className="relative">
                        <select value={filterRamin} onChange={(e) => setFilterRamin(e.target.value)} className="w-full appearance-none border border-slate-200 rounded-lg pl-3 pr-8 py-2 text-sm focus:ring-emerald-500 bg-slate-50">
                          <option value="">Semua Ramin</option>
                          {availableRamins.map(r => <option key={r} value={r}>Ramin {r}</option>)}
                        </select>
                    ...
                    <div className="relative flex-1 sm:w-64">
                      <input type="text" placeholder="Cari nama, ID, atau No. rumah..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full border border-slate-200 rounded-lg pl-9 pr-4 py-2 text-sm focus:ring-emerald-500" />
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-slate-50 text-slate-400 text-xs font-semibold uppercase tracking-wider border-b border-slate-100">
                        ...
                        <th className="p-4">Sektor Ramin</th>
                        <th className="p-4">No. Rumah</th>
                        <th className="p-4 text-center">Baki Mata</th>
                        <th className="p-4 text-center">Total Kitaran</th>
                        {userRole === 'admin' && <th className="p-4 text-center">Tindakan</th>}
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100 text-sm">
                        ...
                        <tr key={r.id} className={`hover:bg-slate-50/80 transition-colors ${r.redeemRequest ? 'bg-amber-50/50' : ''}`}>
                          ...
                          <td className="p-4 font-medium text-slate-800">
                            <div className="flex items-center space-x-2">
                              <span>{r.name}</span>
                              {r.redeemRequest && <span className="bg-amber-100 text-amber-800 text-[10px] px-2 py-0.5 rounded-full font-bold">Minta Tebus</span>}
                            </div>
                          </td>
                      ...
                      {filteredResidents.length === 0 && (
                        <tr><td colSpan="7" className="p-8 text-center text-slate-400">Tiada rekod penduduk ditemui.</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
                ...
                <div className="space-y-4">
                  ...
                  {recentActivity.length === 0 && (
                    <div className="text-center py-8 text-slate-400 text-sm">Menunggu isyarat perkakasan IoT luaran...</div>
                  )}
                </div>
              </div>
            </div>
        ...
        {activeTab === 'statistik' && (
          <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6 animate-in fade-in duration-300">
            <div className="flex justify-between items-center mb-6">
              <div>
                <h2 className="text-2xl font-bold text-slate-800">Prestasi Hijau Sektor Ramin</h2>
                <p className="text-sm text-slate-500">Analisis perbandingan jumlah botol yang berjaya dikumpul mengikut zon jalan.</p>
              </div>
              <select value={statViewType} onChange={(e) => setStatViewType(e.target.value)} className="border border-slate-200 rounded-lg px-3 py-1.5 text-sm font-medium bg-slate-50">
                <option value="keseluruhan">Statistik Keseluruhan</option>
                <option value="semasa">Baki Aktif Semasa</option>
              </select>
            </div>
                ...
                return (
                  <div key={rNum} className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-slate-500 font-bold">{totalTarget} Botol ({percentage}%)</span>
                    </div>
