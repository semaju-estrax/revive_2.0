...
const initialStudents = [
  { id: "S001", name: "Fatihah", marks: 102, form: "1", className: "A", redeemRequest: false },
  { id: "S002", name: "Aisyah", marks: 5, form: "1", className: "B", redeemRequest: false },
  { id: "S003", name: "Zahirah", marks: 8, form: "1", className: "C", redeemRequest: false },
  { id: "S004", name: "Fatimah", marks: 15, form: "1", className: "D", redeemRequest: false },
  { id: "S005", name: "Atiqah", marks: 3, form: "1", className: "E", redeemRequest: false },
  { id: "S006", name: "Hafiya", marks: 25, form: "2", className: "A", redeemRequest: false },
  { id: "S007", name: "Aliya", marks: 0, form: "2", className: "B", redeemRequest: false },
  { id: "S008", name: "Ardaeya", marks: 14, form: "2", className: "C", redeemRequest: false },
  { id: "S009", name: "Syafiqah", marks: 33, form: "4", className: "A", redeemRequest: false },
  { id: "S010", name: "Haneef", marks: 7, form: "4", className: "B", redeemRequest: false },
  { id: "S011", name: "Ayyub", marks: 105, form: "4", className: "C", redeemRequest: true }, 
  { id: "S012", name: "Syazriel", marks: 45, form: "4", className: "D", redeemRequest: false },
  { id: "S013", name: "Haikal", marks: 18, form: "5", className: "A", redeemRequest: false },
  { id: "S014", name: "Zulfikar", marks: 0, form: "5", className: "B", redeemRequest: false },
  { id: "S015", name: "Adeeb", marks: 21, form: "5", className: "C", redeemRequest: false },
  { id: "S016", name: "Iqbal", marks: 11, form: "5", className: "D", redeemRequest: false },
  { id: "S017", name: "Aqil", marks: 3, form: "6", className: "Atas", redeemRequest: false },
  { id: "S018", name: "Ayuni", marks: 27, form: "6", className: "Atas", redeemRequest: false },
  { id: "S019", name: "Zulaiqah", marks: 10, form: "6", className: "Bawah", redeemRequest: false },
  { id: "S020", name: "Husna", marks: 16, form: "6", className: "Bawah", redeemRequest: false },
  { id: "S021", name: "Hafiy", marks: 50, form: "1", className: "E", redeemRequest: false },
  { id: "S022", name: "Dayana", marks: 38, form: "2", className: "A", redeemRequest: false },
  { id: "S023", name: "Nisreen", marks: 4, form: "3", className: "D", redeemRequest: false },
  { id: "S024", name: "Nabilah", marks: 19, form: "4", className: "B", redeemRequest: false },
  { id: "S025", name: "Sofia", marks: 8, form: "5", className: "C", redeemRequest: false }
];
...
export default function App() {
  ...
  // ==========================================
  const totalBottles = students.reduce((sum, student) => sum + student.marks, 0);
  const activeUsers = students.filter(s => s.marks > 0).length;
  const topStudent = students.length > 0 ? [...students].sort((a, b) => b.marks - a.marks)[0] : null;
  ...
  students.forEach(student => {
    ...
    statsByForm[form].total += student.marks;
    if (!statsByForm[form].classes[className]) statsByForm[form].classes[className] = 0;
    statsByForm[form].classes[className] += student.marks;
  });
  ...
  const requestRedeem = async () => {
    if (window.confirm("Adakah anda pasti mahu memohon penebusan botol? Admin akan mengesahkan pemohonan ini.")) {
      try {
  ...
  const handleAddStudent = async (e) => {
    ...
    try {
      await setDoc(doc(db, 'students', newStudent.id.toUpperCase()), {
        id: newStudent.id.toUpperCase(), name: newStudent.name, form: newStudent.form, className: newStudent.className, marks: 0, redeemRequest: false
      });
  ...
  // KEMBALIKAN FUNGSI YANG HILANG DI SINI
  ...
  const simulateESP32Update = async () => {
    ...
    try { await updateDoc(doc(db, 'students', student.id), { marks: student.marks + 1 }); } 
    ...
    reader.onload = async (event) => {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        if (cols.length >= 2 && cols[0].toLowerCase().trim() !== 'id' && cols[0].toLowerCase().trim() !== 'id pelajar') {
          ...
          if (id && name) {
            await setDoc(doc(db, 'students', id), { id, name, form, className, marks: 0, redeemRequest: false });
            successCount++;
          }
        }
      }
        ...
        <div className="fixed inset-0 bg-slate-900 bg-opacity-50 flex justify-center items-center z-50 p-4">
          <div className="bg-white p-6 rounded-xl w-full max-w-md">
            ...
            <form onSubmit={handleUpdateStudent} className="space-y-4">
              ...
              <div><label className="text-sm font-bold text-slate-600">Botol</label><input type="number" required value={editStudentData.marks} onChange={(e) => setEditStudentData({...editStudentData, marks: e.target.value})} className="w-full p-2 border rounded font-bold text-blue-600" /></div>
                  ...
                  <table className="w-full text-left border-collapse min-w-[600px]">
                    <thead className="bg-slate-50 sticky top-0 shadow-sm z-10">
                      <tr>
                        ...
                        <th className="p-4 font-semibold text-slate-600 text-sm text-center">Botol</th>
                        <th className="p-4 font-semibold text-slate-600 text-sm text-center">Status / Nilai</th>
                    ...
                    <tbody>
                      ...
                      ) : (<tr><td colSpan={userRole === 'admin' ? 7 : (userRole === 'student' ? 5 : 6)} className="p-8 text-center text-slate-500">Tiada pelajar dijumpai.</td></tr>)}
                    </tbody>
                  </table>
                </div>
              </div>
          ...
          <div className="animate-in fade-in duration-300">
            <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6 mb-8 flex flex-col md:flex-row justify-between items-center gap-4">
              <div>
                ...
                <p className="text-slate-500 text-sm mt-1">Sumbangan kitar semula sekolah.</p>
              </div>
